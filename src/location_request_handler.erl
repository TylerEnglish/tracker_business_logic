-module(location_request_handler).
...
handle_post(Req, State) ->
    ...
    Result = package_server:location_request(PackID),
    io:format("Result from package_server: ~p~n", [Result]),
    Req3 = case Result of
        {ok, Lon, Lat} ->
            cowboy_req:reply(200, #{<<"content-type">> => <<"application/json">>}, io_lib:format("{\"longitude\":~p,\"latitude\":~p}", [Lon, Lat]), Req2);
        {error, Reason} ->
...
to_html(Req, State) ->
    Req2 = cowboy_req:reply(200, #{<<"content-type">> => <<"text/html">>}, <<"Location Request Handler">>, Req),
    {stop, <<"Location Request Handler">>, Req2, State}.
